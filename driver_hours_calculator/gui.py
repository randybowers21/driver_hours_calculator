...
class UserInterface(customtkinter.CTk):
    def __init__(self):
        ...
        self.title_label = customtkinter.CTkLabel(
            self.main_frame, text="Driver Hours Calculator", font=customtkinter.CTkFont(size=40, weight="bold"))
        self.title_label.grid(
            row=0, column=0, columnspan=2, padx=20, sticky='nsew')

        self.file_frame = customtkinter.CTkFrame(
            self.main_frame, corner_radius=0)
        self.file_frame.columnconfigure(0, weight=1)
        self.file_frame.grid(row=2, column=0, columnspan=3,
                             sticky='nsew', pady=5)

        self.file_label = customtkinter.CTkLabel(
            self.file_frame, text='Upload Raw Driver Hours CSV: ', font=customtkinter.CTkFont(size=30, weight='bold'))
        ...
        self.get_file_button = customtkinter.CTkButton(self.file_frame, text='Browse', font=customtkinter.CTkFont(
            size=15, weight='bold'), command=self.open_file_search_dialog)
        self.get_file_button.grid(
        ...
        self.email_frame.grid(
            row=3, column=0, columnspan=3, sticky='nsew', pady=5)

        self.email_label = customtkinter.CTkLabel(
            self.email_frame, text='Email Options: ', font=customtkinter.CTkFont(size=30, weight='bold'))
        self.email_label.grid(
            row=0, column=0, columnspan=3, padx=20, sticky='nsw')
        self.email_recipients_label = customtkinter.CTkLabel(
            self.email_frame, text='Recipients: ', font=customtkinter.CTkFont(size=15, weight='bold'))
        ...
        self.email_recipients_entry.grid(
        ...
        self.send_email_checkbox = customtkinter.CTkCheckBox(
            self.email_frame, text='Send Email?', command=self.enable_disable_email)
        ...
        self.create_report_button.grid(
            row=4, column=0, columnspan=2, padx=5, pady=10, sticky='nsew')
    ...
    def create_final_hours_report(self):
        ...
        if self.file_name:
            ...
            file_name = final_report.generate_hours_report(SAVE_FILE=True)
            if send_email:
